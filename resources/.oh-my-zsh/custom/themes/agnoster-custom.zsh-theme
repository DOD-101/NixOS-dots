...
prompt_dir() {
    # changed here to limit prompt to only show current and two level above for prompt path 
  if [ $(echo $(( $(pwd | tr -cd '/' | wc -c) - 1 ))) -gt 4 ]; then
    prompt_segment yellow $CURRENT_FG '../%3~'
  else 
